package com.jdbc.dynamicdatasource.register;

import com.alibaba.druid.pool.DruidDataSource;
import com.jdbc.dynamicdatasource.config.DynamicDataSourceContextHolder;
import com.jdbc.dynamicdatasource.config.DynamicRoutingDataSource;
...
public class DynamicDataSourceRegister implements ImportBeanDefinitionRegistrar, EnvironmentAware {
    ...
    public void registerBeanDefinitions(AnnotationMetadata annotationMetadata, BeanDefinitionRegistry beanDefinitionRegistry) {
        ...
        for (int i = 0; i < configs.size(); i++) {
            ...
            DynamicDataSourceContextHolder.slaveDataSourceIds.add(key);
        ...
        logger.info("注册数据源成功，一共注册{}个数据源", DynamicDataSourceContextHolder.dataSourceIds.size());
    }
    ...
    private Class<? extends DataSource> getDataSourceType(String typeStr) {
        ...
        try {
            if (StringUtils.hasLength(typeStr)) {
                ...
                // 默认为druid数据源，与springboot默认数据源保持一致
